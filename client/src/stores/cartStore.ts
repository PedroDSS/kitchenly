  ...
  actions: {
    ...
    async updateQuantity(productId, quantity) {
      try {
        ...
        if (existingItem) {
          const response = await axios.put('api/cart', { cartItemId: existingItem.id, quantity });
    ...
    async generatePaymentLink() {
      try {
        ...
        const response = await axios.post('api/payments/unique-payment-link', {
          items,
          userId: userId.toString(),
        });
    ...
    async applyPromoCode(code) {
      try {
        const response = await axios.post('api/promocodes/validate', { code });
